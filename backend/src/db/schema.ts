...
import { sqliteTable, text, integer, primaryKey, check
...
export const usersTable = sqliteTable('users', {
  userId: text('user_id').primaryKey().$defaultFn(() => uuidv4()),
...
export const rolesTable = sqliteTable('roles', {
  roleId: text('role_id').primaryKey().$defaultFn(() => uuidv4()),
  roleName: text('role_name').notNull().unique(),
});

export const locationsTable = sqliteTable('locations', {
  locationId: text('location_id').primaryKey().$defaultFn(() => uuidv4()),
  locationName: text('location_name').notNull().unique(),
});

export const companiesTable = sqliteTable('companies', {
  companyId: text('company_id').primaryKey().$defaultFn(() => uuidv4()),
  companyName: text('company_name').notNull().unique(),
});


export const userPreferencesTable = sqliteTable('user_preferences', {
  preferenceId: text('preference_id').primaryKey().$defaultFn(() => uuidv4()),
  userId: text('user_id').references(() => usersTable.userId, { onDelete: 'cascade' }).notNull(),
...
export const jobsTable = sqliteTable('jobs', {
  jobId: text('job_id').primaryKey().$defaultFn(() => uuidv4()),
...
export const jobRecommendationsTable = sqliteTable('job_recommendations', {
  recommendationId: text('recommendation_id').primaryKey().$defaultFn(() => uuidv4()),
  ...
  userId: text("user_id")
    ...
    .notNull(),
  jobId: text("job_id").references(() => jobsTable.jobId).notNull(),
}
);